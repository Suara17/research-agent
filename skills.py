...
class SkillIntegrationTools:
    ...
    def execute_script(self, skill_name: str, args: Optional[Dict] = None) -> str:
        """
        Execute a script provided by the skill (synchronous version).

        This function looks for an executable script in the skill directory:
        - run.py (Python script)
        - run.sh (Shell script, Unix-like systems)
        - run.bat (Batch script, Windows)

        Args:
            skill_name (str): The name of the skill.
            args (dict, optional): Arguments to pass to the script as JSON.
        ...
        try:
            ...
            # Look for executable scripts in order of preference
            script_candidates = ["run.py", "run.sh", "run.bat"]
            script_path = None
            for candidate in script_candidates:
                candidate_path = skill_root / candidate
                if candidate_path.exists():
                    script_path = candidate_path
                    break

            if not script_path:
                return f"<stdout></stdout><stderr>Error: No executable script found in skill '{skill_name}'. Expected one of: {', '.join(script_candidates)}</stderr>"
            ...
            if script_path.name == "run.py":
                # Python script
                if args:
                    # 使用临时文件传递参数，避免环境变量转义问题
                    temp_args_file = tempfile.NamedTemporaryFile(
                        mode='w',
                        encoding='utf-8',
                        suffix='.json',
                        delete=False
                    )
                    json.dump(args, temp_args_file, ensure_ascii=False, indent=2)
                    temp_args_file.close()
                    env["SKILL_ARGS_FILE"] = temp_args_file.name
                    logger.info(f"[SKILL_DEBUG] Created temp args file: {temp_args_file.name}")

                command = [sys.executable, str(script_path)]

            elif script_path.name in ("run.sh", "run.bat"):
                # Shell/Batch script
                if args:
                    temp_args_file = tempfile.NamedTemporaryFile(
                        mode='w',
            ...
            logger.info(f"Executed script {script_path.name} in skill {skill_name} successfully")
            ...
            logger.warning(
                f"Failed to execute script in skill {skill_name}: {str(e)}",
                exc_info=True,
            )
