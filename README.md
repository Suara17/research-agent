...
2. **配置环境变量**：
   复制 `.env.example` 为 `.env`，并填入你的 `DASHSCOPE_API_KEY` 和搜索服务密钥（`SERPAPI_API_KEY` 或 `BING_API_KEY`）。
...
4. **批量验证**：
   ...
   python run_batch.py
   ```
   该脚本会读取 `question.jsonl`，调用本地服务生成答案，并输出到 `submission.jsonl`。同时生成 `batch_run.log` 记录详细运行日志。

### 部署到 PAI-EAS

1. **打包项目**：
   在 LangStudio 中，将整个项目目录打包为 ZIP 文件。确保包含 `requirements.txt` 和 `agent.py`。

2. **创建服务**：
   - 上传 ZIP 包。
   - 启动命令：`python agent.py` (或使用 `uvicorn agent:app --host 0.0.0.0 --port 8000`)。
   - 端口：`8000`。

3. **配置环境变量**：
   在 EAS 服务配置中，添加 `DASHSCOPE_API_KEY` 和搜索服务密钥。

4. **验证服务**：
   部署完成后，使用获取的 Endpoint 和 Token 进行调用测试（参考上文“服务部署调试”部分）。
