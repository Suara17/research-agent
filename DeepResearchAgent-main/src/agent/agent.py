...
# Define Agent alias or base class
...
async def build_agent(config,
    ...
    # Build MCP Tools - Skipped for lightweight version
    ...
    if default_managed_agents is None:
        ...
        for managed_agent in default_managed_agents:
            if managed_agent.name not in used_managed_agents:
                ...
                managed_agent_tools.append(managed_agent)
    ...
    # Use AGENT.build with 'type'
    agent = AGENT.build(dict(type=agent_config.type, **agent_config_dict))
