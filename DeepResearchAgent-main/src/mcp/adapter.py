...
try:
    ...
    FastMCPTool = Any # Placeholder

import json
import keyword
import re
# import inflection # Removed

from src.tools import AsyncTool

def _to_snake_case(name: str) -> str:
    """Simple replacement for inflection.underscore"""
...
class ToolAdapter(ABC):
    def adapt(
        self,
        client,
        tool: Any # Was Tool (arg name?) or type hint? Assuming arg name based on context
    ) -> Any:
        pass
    ...
    async def adapt(
        ...
        class MCPAdaptTool(AsyncTool):
            ...
            async def forward(self, *args, **kwargs) -> str:
                ...
                return json.loads(mcp_output[0].text)

        name = _to_snake_case(tool.name)  # Convert to snake_case, e.g., "GetWeatherTool" -> "get_weather_tool"
