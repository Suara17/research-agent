...
from src.models.base import InferenceClientModel
...
class ModelManager(metaclass=Singleton):
    ...
    def init_models(self, use_local_proxy: bool = False):
        ...
        # self._register_langchain_models(use_local_proxy=use_local_proxy) # Deprecated
        # self._register_vllm_models(use_local_proxy=use_local_proxy) # Deprecated
        # self._register_deepseek_models(use_local_proxy=use_local_proxy) # Deprecated
    ...
    def _register_qwen_models(self, use_local_proxy: bool = False):
        ...
        logger.info(f"Initializing Qwen model with ID: {model_id}")
        
        # Priority: IFLOW_API_KEY (Temporary first choice) -> DASHSCOPE_API_KEY
        if os.getenv("IFLOW_API_KEY"):
            api_key = os.getenv("IFLOW_API_KEY")
            api_base = "https://apis.iflow.cn/v1"
            logger.info("Using IFLOW API for Qwen models")
        else:
            api_key = self._check_local_api_key(local_api_key_name="DASHSCOPE_API_KEY", 
                                                remote_api_key_name="DASHSCOPE_API_KEY")
            api_base = "https://dashscope.aliyuncs.com/compatible-mode/v1"
            
            if api_key == PLACEHOLDER:
                 logger.warning("DASHSCOPE_API_KEY not found. Qwen models might fail if not using a proxy.")
