...
def final_verification():
    ...
    try:
        ...
        if response.status_code == 200:
            print("   ✅ API 访问正常 (返回 200 状态码)")
            # 检查是否返回有效 JSON
            try:
                data = response.json()
                print(f"   ✅ 返回有效 JSON 数据，包含 {len(data.get('results', []))} 个结果")
            except:
                print("   ❌ 返回的数据不是有效 JSON")
        else:
            print(f"   ❌ API 访问失败 (返回 {response.status_code} 状态码)")
    ...
    try:
        ...
        if results is not None:
            print(f"   ✅ Python 函数正常工作，返回 {len(results)} 个结果")
            if results:
                print(f"   ✅ 第一个结果标题: {results[0].get('title', 'N/A')}")
        else:
            print("   ⚠️  Python 函数返回 None")
    except Exception as e:
        print(f"   ❌ Python 函数异常: {e}")
    ...
    try:
        with open("searxng/settings.yml", "r", encoding="utf-8") as f:
            ...
            if "limiter: false" in content:
                print("   ✅ settings.yml 中已设置 limiter: false")
            else:
                print("   ❌ settings.yml 中未找到 limiter: false")
                
            if "formats:" in content and "json" in content.split("formats:")[-1][:200]:
                print("   ✅ settings.yml 中已添加 json 格式支持")
            else:
                print("   ❌ settings.yml 中未找到 json 格式支持")
    except Exception as e:
        print(f"   ❌ 读取配置文件异常: {e}")
    ...
    try:
        with open("research_agent/search.py", "r", encoding="utf-8") as f:
            ...
            if "X-Forwarded-For" in content and "X-Real-IP" in content:
                print("   ✅ 代码中已添加适当的 HTTP 头")
            else:
                print("   ❌ 代码中未找到适当的 HTTP 头")
    except Exception as e:
        print(f"   ❌ 读取代码文件异常: {e}")
